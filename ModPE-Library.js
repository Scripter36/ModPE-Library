 ...
 */
ModPE.setTimeout = function(func, time) {
    ...
    ModPE.threads.push(thread);
    thread.start();
    return ModPE.threads.length - 1;
};
 ...
 */
ModPE.clearTimeout = function(number) {
    ModPE.threads[number].interrupt();
    ModPE.threads.splice(number, 1);
};
 ...
 * @return {Number}       thread number
 */
ModPE.setInterval = function(func, time) {
    var thread = new java.lang.Thread(new java.lang.Runnable({
        run: function() {
            try {
 ...
 * @param  {Number} number thread number
 */
ModPE.clearInterval = function(number) {
    ModPE.threads[number].interrupt();
    ModPE.threads.splice(number, 1);
};
